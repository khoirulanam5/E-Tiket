...
<script>
  ...
  function ISI_SELECT(){
    ...
    $.ajax({
      url: "<?= site_url('bus/getKategoriBus') ?>",
      type: "GET",
      dataType: "JSON",
      success: function(data){
        console.log(data)
  ...
  function REFRESH_DATA(){
    ...
    var tb_data = $("#tb_data").DataTable({
      ...
      "ajax": {
          "url": "<?= site_url('bus/getAllData') ?>", // URL untuk mengambil data
          "type": "GET"
      },
      "columns": [
          {
          ...
          { "data": "id_bus" }, // Menampilkan ID Bus
          { "data": "nm_jenis_bus" }, // Menampilkan Jenis Bus
          { "data": "no_pol" }, // Menampilkan No. Polisi
          { "data": "jumlah_kursi", class : "text-center" }, // Menampilkan Jumlah Kursi
          { "data": "foto", 
            ...
            "render" : function(data){
              return "<div class='dropdown'>" +
                       ...
                       "<div class='dropdown-menu' aria-labelledby='dropdownMenuButton'>" +
                         ...
                         "<a class='dropdown-item' href='javascript:void(0)' onclick='deleteData(\""+ data.id_bus +"\");'><i class='fas fa-trash'></i> Delete</a>" +
                       "</div>" +
  ...
  $("#FRM_DATA").on('submit', function(event){
    ...
    if(save_method == 'save') {
        urlPost = "<?= site_url('bus/saveData') ?>"; // URL untuk menyimpan data baru
    } else {
        urlPost = "<?= site_url('bus/updateData/') ?>" + id_edit; // URL untuk memperbarui data
    }
  ...
  function editData(data){
    ...
    id_edit = data.id_bus; // Set ID data yang akan diedit
    $("#modal_add .modal-title").text('Edit Data'); // Mengubah judul modal menjadi Edit Data
    $("[name='id_jenis_bus']").val(data.id_jenis_bus); // Mengisi dropdown Jenis Bus
    $("[name='id_kategori']").val(data.id_kategori); // Mengisi dropdown Kategori Bus
    $("[name='jumlah_kursi']").val(data.jumlah_kursi); // Mengisi jumlah kursi
    $("[name='no_pol']").val(data.no_pol); // Mengisi no polisi
  ...
  function deleteData(id){
    ...
    urlPost = "<?= site_url('bus/deleteData') ?>"; // URL untuk menghapus data
    formData = "id_bus=" + id; // Menyiapkan data yang akan dikirim
