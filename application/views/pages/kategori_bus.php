        ...
        <div class="col-6">
          <div class="card" style="margin-top: 1rem">
            <div class="card-header">
              <h3 class="card-title">Data Kategori Bus</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
          ...
          <form id="FRM_DATA">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" name="nm_kategori" rows="2"></textarea>
                </div>
              </div>
...
<script>
  ...
  $(function () {
    ...
    $("#BTN_SAVE").click(function(){
      ...
      if(save_method == 'save') {
          urlPost = "<?= site_url('kategori_bus/saveData') ?>"; // URL untuk save data baru
      }else{
          urlPost = "<?= site_url('kategori_bus/updateData') ?>"; // URL untuk update data
          formData+="&id_kategori="+id_edit // Menambahkan ID data yang akan diedit
      }
      ...
      "columns": [
          ...
          { "data": null, 
            "render" : function(data){
              ...
              return `
                <div class="btn-group">
                  <button class="btn btn-sm btn-primary" type="button" data-toggle="dropdown">Aksi</button>
                  <ul class="dropdown-menu">
                    <a class="dropdown-item" href="#" onclick='editData(${JSON.stringify(data)});'><i class='fas fa-edit'></i> Edit</a>
                    <a class="dropdown-item" href="#" onclick='deleteData("${data.id_kategori}");'><i class='fas fa-trash'></i> Delete</a>
                  </ul>
                </div>
              `
            },
  ...
  function editData(data){
    ...
    id_edit = data.id_kategori;

    $("#modal_add .modal-title").text('Edit Data') // Mengubah judul modal
    $("[name='id_kategori']").val(data.id_kategori) // Mengisi form dengan data
    $("[name='nm_kategori']").val(data.nm_kategori) // Mengisi form dengan data
    $("#modal_add").modal('show') // Menampilkan modal
  }
  ...
  function deleteData(id){
    ...
    urlPost = "<?= site_url('kategori_bus/deleteData') ?>"; // URL untuk delete data
